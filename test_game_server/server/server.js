...
io.on('connection', (sock) => { //
  ...
  sock.on('message', (text) => io.emit('message', users[sock.id] + ": " + text)); // calling io.emit means send to everyone including client. so a broadcast to everyone in server.
  ...
  sock.on('turn',({x,y}) => {
    if (cooldown()){
      ...
      if(playerWon) {
        ...
        var select_statement = `SELECT wins FROM userinfo WHERE username = '` + users[sock.id] + `'`;
        ...
        client.query(select_statement, (err, res) => {
          if(!err){
            ...
            var update_statement = `UPDATE userinfo SET wins = ` + (currentWins) + ` WHERE username = '` + users[sock.id] + `'`;
  ...
  sock.on('new-user', name => {
    ...
    var insert_statement = `INSERT INTO userinfo (username, wins) VALUES ('` + unique_name + `', 0)`;
